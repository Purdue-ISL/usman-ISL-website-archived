...
module JB
  class Path
    ...
    Paths = {
      ...
      :posts => "_posts",
      :publications => "publications/_posts"
    }
...
desc "Post a new paper in #{CONFIG['publications']}"
task :paper do
  abort("rake aborted: '#{CONFIG['publications']}' directory not found.") unless FileTest.directory?(CONFIG['publications'])
  title = ENV["title"] || "new-post"
  tags = ENV["tags"] || "[]"
  #category = ENV["category"] || ""
  #category = "\"#{category.gsub(/-/,' ')}\"" if !category.empty?
  ...
  filename = File.join(CONFIG['publications'], "#{date}-#{slug}.#{CONFIG['paper_ext']}")
  if File.exist?(filename)
    abort("rake aborted!") if ask("#{filename} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
  end
